...
- `retries=N` on an **agent** phase = extra gate-correction rounds re-sent into the same coding-agent session, so context stays intact. Code-phase re-execution is not implemented.
