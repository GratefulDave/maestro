...
On violations the harness does **not** restart the agent — it sends the violation list back into the same coding-agent session as a correction, bounded by that phase's `retries`. Every gate result is traced to the `gate_results` table. Exhausting the retries raises `GateFailure` and fails the phase.
...
- **Phase retries vs. fix loops.** `retries=N` on `PhaseParams` re-attempts one agent phase's gate corrections in the same session. Code-phase re-execution is not implemented. A fix loop is a *chain* of phases repeated — different agents, new envelopes each pass.
