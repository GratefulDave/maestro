...
def _runner(*prefix: str) -> "rr.ResolvedRunner":
    ...
    return rr.ResolvedRunner(
        runner="pytest",
        executable=prefix[0],
        launcher_args=tuple(prefix[1:]),
        origin="declared",
        probe_exit=5,
        version="unittest-stub",
    )
...
INTEGRATION_RUNNER = _runner(
    sys.executable, "-m", "unittest", "discover", "-q", "-s", ".", "-p", "test_*.py"
)
...
class TwoBasesAndIdentity(WorktreeTestCase):
    ...
    def test_branch_and_worktree_carry_the_run_node_attempt_tuple(self):
        ...
        self.assertEqual(
            _git(attempt.path, "symbolic-ref", "--short", "HEAD"), attempt.branch
        )
...
class PermissionCheck(WorktreeTestCase):
    ...
    def test_tampering_with_provisioned_untracked_content_convicts_whatever_the_globs_say(
        self,
    ):
    ...
    def test_a_rewritten_provisioned_file_with_restored_stat_metadata_still_convicts(
        self,
    ):
...
class SchedulerSideCommit(WorktreeTestCase):
    ...
    def test_the_private_index_lives_outside_the_worktree_and_the_scratch(self):
        ...
        attempt = _attempt(self.repo, self.root)
        self.assertFalse(
            str(attempt.private_index).startswith(str(attempt.path) + os.sep)
        )
        self.assertFalse(
            str(attempt.private_index).startswith(str(attempt.scratch) + os.sep)
        )

    def test_the_commit_set_is_exactly_the_measured_delta(self):
        attempt, _, _, _, sha = self._committed()
        listed = _git(
            attempt.path, "diff-tree", "--no-commit-id", "--name-only", "-r", sha
        ).splitlines()
    ...
    def test_the_recorded_base_is_the_commit_s_sole_parent(self):
        attempt, _, _, _, sha = self._committed()
        self.assertEqual(
            _git(attempt.path, "rev-list", "--parents", "-n", "1", sha).split()[1:],
            [attempt.base],
        )
    ...
    def test_a_write_between_the_after_inventory_and_staging_fails_the_staging_assertion(
        self,
    ):
    ...
    def test_a_node_with_an_empty_delta_still_gets_an_output_sha(self):
        ...
        self.assertEqual(
            _git(attempt.path, "diff-tree", "--no-commit-id", "--name-only", "-r", sha),
            "",
        )
    ...
    def test_the_worktree_index_is_refreshed_to_the_committed_tree(self):
        ...
        tracked_dirt = [
            line
            for line in _git(attempt.path, "status", "--porcelain").splitlines()
            if not line.startswith("??")
        ]
        self.assertEqual(tracked_dirt, [])


# ── repeated immutable candidates in one attempt ────────────────────────────


class DescendantCandidates(WorktreeTestCase):
    def _first_candidate(self) -> tuple[wt.AttemptWorktree, str]:
        attempt = _attempt(self.repo, self.root)
        baseline = wt.take_baseline(attempt)
        (attempt.path / "calc.py").write_text("def add(a, b):\n    return a + b\n")
        after = wt.inventory(attempt.path)
        first = wt.commit_measured_delta(
            attempt, wt.delta(baseline, after), after, "nodeA: first candidate"
        )
        return attempt, first

    def test_second_candidate_reuses_the_attempt_and_descends_from_the_first(self):
        attempt, first = self._first_candidate()
        identity = (
            attempt.path,
            attempt.branch,
            attempt.scratch,
            attempt.private_index,
            attempt.run_id,
            attempt.node_id,
            attempt.attempt_no,
        )
        self.assertIsNotNone(attempt.baseline)
        self.assertIsNotNone(attempt.ignored_at_base)

        wt.prepare_descendant_candidate(attempt, first)

        self.assertEqual(attempt.base, first)
        self.assertEqual(
            attempt.tracked_at_base,
            frozenset(wt.inventory_at_commit(attempt.repo, first)),
        )
        self.assertIsNone(attempt.baseline)
        self.assertIsNone(attempt.ignored_at_base)
        self.assertEqual(
            (
                attempt.path,
                attempt.branch,
        ...
        self.assertEqual(_git(attempt.path, "rev-parse", "HEAD"), first)
        self.assertEqual(
            wt.attempt_ref_commit(
                self.repo, attempt.run_id, attempt.node_id, attempt.attempt_no
            ),
            first,
        )
        ...
        (attempt.path / "calc.py").write_text("def add(a, b):\n    return a - b\n")
        after = wt.inventory(attempt.path)
        second = wt.commit_measured_delta(
            attempt, wt.delta(baseline, after), after, "nodeA: second candidate"
        )

        self.assertNotEqual(second, first)
        self.assertEqual(
            _git(attempt.path, "rev-list", "--parents", "-n", "1", second).split()[1:],
            [first],
        )
        self.assertNotEqual(
            _git(attempt.path, "rev-parse", f"{first}^{{tree}}"),
            _git(attempt.path, "rev-parse", f"{second}^{{tree}}"),
        )

    def test_prepare_refuses_a_stale_requested_parent(self):
        attempt, first = self._first_candidate()
        stale_parent = attempt.base

        with self.assertRaises(wt.HeadMoved):
            wt.prepare_descendant_candidate(attempt, stale_parent)
    ...
    def test_prepare_refuses_a_stale_attempt_ref(self):
        attempt, first = self._first_candidate()
        _git(attempt.path, "checkout", "-q", "--detach", first)
        _git(attempt.repo, "update-ref", attempt.ref, attempt.base, first)
    ...
    def test_prepare_refuses_a_head_that_does_not_match_the_candidate(self):
        attempt, first = self._first_candidate()
        _git(attempt.path, "checkout", "-q", "--detach", attempt.base)
    ...
    def test_prepare_requires_a_full_object_digest(self):
        attempt, first = self._first_candidate()

        with self.assertRaises(wt.InvalidCandidateParent):
            wt.prepare_descendant_candidate(attempt, first[:12])
...
class FourChecks(WorktreeTestCase):
    ...
    def test_an_identically_named_agent_created_file_does_convict(self):
        ...
        (attempt.path / ".pre_gate_cache").write_text(
            "written by the agent, after settle\n"
        )
        verdict = wt.check_post_commit(attempt, expected)
        self.assertFalse(verdict.ok)
        self.assertEqual(
            [d.path for d in verdict.cleanliness.divergences], [".pre_gate_cache"]
        )
    ...
    def test_a_relocated_byproduct_is_judged_on_its_tuple_not_on_its_name(self):
        ...
        attempt, expected, _ = self._verified_bracket(pre_gate_writes=".pre_gate_cache")
        (attempt.path / ".pre_gate_cache").rename(
            attempt.path / ".pre_gate_cache_moved"
        )
        verdict = wt.check_post_commit(attempt, expected)
        self.assertFalse(verdict.ok)
        self.assertEqual(
            [d.path for d in verdict.cleanliness.divergences],
    ...
    def test_post_gate_residue_is_reported_at_pre_merge_rather_than_convicting(self):
        ...
        self.assertEqual(
            [d.path for d in verdict.cleanliness.divergences], ["coverage.xml"]
        )
...
class GateScope(WorktreeTestCase):
    ...
    def test_a_multi_path_selector_reaches_the_runner_as_separate_paths(self):
        ...
        gate = wt.run_node_gate(
            ...
            _runner(sys.executable),
            ("-c", script, "tests/a.py"),
            selector="tests/a.py tests/b.py",
            cancel_requested=lambda: False,
        )
        self.assertEqual(json.loads(gate.tail[-1]), ["tests/a.py", "tests/b.py"])
...
class MergeOrder(unittest.TestCase):
    ...
    def test_an_abandoned_node_cascades_the_same_way(self):
        nodes = [
            wt.NodeRecord("nodeA", 1, (), "CANCELLED"),
            wt.NodeRecord("nodeC", 2, ("nodeA",), "VERIFIED"),
        ]
...
class MergeAndAcceptance(WorktreeTestCase):
    def _integration(self) -> Path:
        ...
        wt.create_worktree(
            self.repo, path, "integration", _git(self.repo, "rev-parse", "HEAD")
        )
        return path

    def _node(
        self, node_id: str, filename: str, body: str
    ) -> tuple[wt.AttemptWorktree, str]:
        attempt = _attempt(self.repo, self.root, node_id=node_id)
        baseline = wt.take_baseline(attempt)
    ...
    def test_two_nodes_merge_in_frontier_order_and_the_integration_gate_is_green(self):
        ...
        _, sha_b = self._node(
            "nodeB", "strutil.py", 'def shout(s):\n    return s.upper() + "!"\n'
        )

        order = wt.merge_frontier(
            [
                wt.NodeRecord("nodeB", 1, (), "VERIFIED"),
        ...
        results = [
            wt.merge_verified_node(integration, node, sha)
    ...
    def test_the_merge_consumes_the_output_sha_and_never_the_branch(self):
        ...
        integration = self._integration()
        attempt, sha = self._node(
            "nodeA", "calc.py", "def add(a, b):\n    return a + b\n"
        )
    ...
    def test_final_acceptance_is_the_deduplicated_union_of_merged_specs(self):
        ...
        nodes = [
            wt.NodeRecord("nodeB", 1, (), "MERGED", specs=("test_strutil", "lint")),
            wt.NodeRecord("nodeA", 1, (), "MERGED", specs=("test_calc", "lint")),
            wt.NodeRecord("nodeC", 1, (), "BLOCKED", specs=("test_never",)),
        ]
        self.assertEqual(
            wt.acceptance_specs(nodes), ("lint", "test_calc", "test_strutil")
        )
    ...
    def test_cleanup_removes_the_worktree_and_branch_after_ancestry(self):
        ...
        wt.remove_attempt_worktree(
            attempt,
            ancestry_proven=result.ancestry_proven,
