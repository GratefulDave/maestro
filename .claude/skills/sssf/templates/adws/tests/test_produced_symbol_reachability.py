...
class UnreferencedSymbolTests(unittest.TestCase):
    ...
    def test_every_unreferenced_symbol_is_reported_not_the_first(self):
        ...
        self.assertEqual(
            names(rc.unreferenced(produced, surface)),
    ...
    def test_a_bare_helper_in_a_test_file_is_not_exempt(self):
        ...
        self.assertEqual(
            names(rc.unreferenced(produced, dict(produced))), ["_unused_helper"]
        )
    ...
    def test_a_symbol_this_attempt_added_to_an_existing_file_is_adjudicated(self):
        base = {"policy.py": "def legacy():\n    return 1\n"}
        produced = {"policy.py": ("def legacy():\n    return 1\n\n_SCRATCH = {}\n")}
        self.assertEqual(
            names(rc.unreferenced(produced, dict(produced), base)), ["_SCRATCH"]
        )
    ...
    def test_methods_are_not_adjudicated(self):
        ...
        surface = dict(
            produced, **{"caller.py": "from policy import Policy\nPolicy()\n"}
        )
        self.assertEqual(rc.unreferenced(produced, surface), ())
...
class AdjudicateReachabilityTests(unittest.TestCase):
    ...
    def test_an_agent_node_is_refused_semantically_with_the_symbols_named(self):
        ...
        self.assertEqual(
            verdict.unreferenced_symbols,
            ("gap_policy.py:7:locator_row_id", "gap_policy.py:7:_LOCATOR_NS"),
        )
    ...
    def test_a_code_node_is_refused_terminally(self):
        ...
        self.assertIs(verdict.block_reason, st.BlockReason.PRODUCED_SYMBOL_UNREFERENCED)
...
class ReachabilityThroughScheduler(SchedulerFixture):
    ...
    def test_an_unreferenced_produced_symbol_refuses_the_attempt(self):
        ...
        report = self.schedule([self.agent("build", outputs=("*.py",))]).run()
        ...
        self.assertEqual(
            rows[0].get("detail", {}).get("unreferenced_symbols"),
    ...
    def test_a_node_whose_symbols_are_all_referenced_merges(self):
        ...
        self.written["build"] = {
            "build.py": ("def apply_gap_policy(rows):\n    return rows\n"),
            "tests/test_build.py": (
        ...
        report = self.schedule(
            [self.agent("build", outputs=("*.py", "tests/*.py"))]
        ).run()
        self.assertEqual(
            self.states(), {"build": "MERGED", "build::review": "ACCEPTED"}
        )
