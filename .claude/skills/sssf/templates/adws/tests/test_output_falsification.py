...
P5_OUTPUTS = (
    "src/lexgenius_pipeline/ingestion/judicial/cmo/gap_policy.py",
...
class OutputsUnnamedByGateTests(unittest.TestCase):
    def test_the_r7_lane_reverts_its_production_file_and_keeps_its_tests(self):
        ...
        self.assertEqual(
            (P5_OUTPUTS[0],), vf.outputs_unnamed_by_gate(P5_OUTPUTS, P5_ARGV)
        )
    ...
    def test_a_node_id_suffix_still_selects_its_file(self):
        ...
        self.assertEqual(
            (),
            vf.outputs_unnamed_by_gate(("tests/t.py",), ("tests/t.py::TestX::test_y",)),
        )
    ...
    def test_a_directory_token_selects_the_files_beneath_it(self):
        self.assertEqual(
            ...
            vf.outputs_unnamed_by_gate(
                ("src/a.py", "tests/unit/test_a.py"), ("tests/unit",)
            ),
        )
    ...
    def test_a_directory_output_is_selected_by_a_token_beneath_it(self):
        ...
        self.assertEqual(
            (), vf.outputs_unnamed_by_gate(("tests",), ("tests/unit/test_a.py",))
        )
    ...
    def test_an_empty_argv_names_nothing(self):
        self.assertEqual(("src/a.py",), vf.outputs_unnamed_by_gate(("src/a.py",), ()))
...
class AdjudicateOutputFalsificationTests(unittest.TestCase):
    ...
    def test_the_counting_rule_is_the_same_one_that_admitted_the_post_gate(self):
        ...
        self.assertTrue(
            vf.adjudicate_output_falsification(
                vf.adjudicate_gate(green(8), 9), ("src/a.py",)
            ).verified
        )
        self.assertFalse(
            vf.adjudicate_output_falsification(
...
class RevertAndRestoreTests(SchedulerFixture):
    ...
    def _attempt_with_work(self, files):
        ...
        output_sha = wt.commit_measured_delta(attempt, measured, after, "a attempt 1")
    ...
    def test_the_restore_returns_the_tree_to_the_sealed_commit(self):
        ...
        self.assertEqual(output_sha, wt.attempt_ref_commit(self.repo, "run1", "a", 1))
...
class GateReadsTheTreeFixture(SchedulerFixture):
    ...
    def agent(self, node_id="p5", depth=0, needs=(), outputs=None, specs=()):
        return st.PlanNode(
            node_id=node_id,
...
class TheLoopTests(GateReadsTheTreeFixture):
    def test_an_honest_node_merges_and_the_gate_ran_three_times(self):
        ...
        report = self.schedule(
            [node],
            deps=self.deps(
                run_node=self.builder(hollow=False), run_gate=self.tree_reading_gate()
            ),
        ).run()
    ...
    def test_a_hollow_test_file_is_refused_however_green_its_gate_was(self):
        ...
        report = self.schedule(
            ...
            deps=self.deps(
                run_node=self.builder(hollow=True), run_gate=self.tree_reading_gate()
            ),
        ).run()
        ...
        self.assertIs(node_row.block_reason, st.BlockReason.SEMANTIC_BUDGET_EXHAUSTED)

    def test_the_refusal_names_the_paths_and_is_semantic(self):
        """§7.5: the retained-session repair receives the falsified paths."""
        self.schedule(
            [self.agent()],
        ...
        rows = sorted(
            self.store.attempts_for("run1", "p5"), key=lambda row: row.attempt_no
        )
        self.assertIs(rows[0].retry_class, st.RetryClass.SEMANTIC)
        self.assertGreaterEqual(len(self.prompts["p5"]), 2)
        repair_prompt = self.prompts["p5"][1]
        self.assertIn(self.PROD, repair_prompt)
        self.assertIn("declared tests do not exercise", repair_prompt)

    def test_the_refused_attempt_is_repaired_without_reimplementation(self):
        """The sealed candidate is handed back to the retained builder.

        This fixture deliberately makes that repair empty. The empty repair
        spends the same lane's semantic budget without minting a candidate or
        a replacement attempt, and the immutable candidate remains the subject
        of every repair request.
        """
        self.schedule(
            [self.agent()],
            config=self.config(semantic_ceiling=2),
            deps=self.deps(
                run_node=self.builder(hollow=True),
                run_gate=self.tree_reading_gate(),
            ),
        ).run()
        ...
        refused_sha = self.store.attempt_sealed_output("run1", "p5", 1)
        self.assertIsNotNone(refused_sha)
        self.assertIn(refused_sha, self.prompts["p5"][1])
        spends = self.store.lane_retry_spends("run1", "p5", limit=100)
        self.assertEqual(
            [st.LaneRetryClass.SEMANTIC, st.LaneRetryClass.SEMANTIC],
            [spend.retry_class for spend in spends],
        )
        self.assertEqual({refused_sha}, {spend.candidate_sha for spend in spends})
        self.assertIs(
            self.store.get_node("run1", "p5").block_reason,
...
class TheCheckHasNoSubjectTests(GateReadsTheTreeFixture):
    ...
    def test_it_does_not_merge_claiming_it_was_verified(self):
        self.schedule(
            [self.agent()],
            config=self.config(semantic_ceiling=1),
            deps=self.deps(run_node=self.builder(), run_gate=self.tree_reading_gate()),
        ).run()

        self.assertNotEqual(self.states()["p5"], st.NodeState.MERGED.value)
        self.assertIs(self.store.get_node("run1", "p5").state, st.NodeState.BLOCKED)
        rows = sorted(
            self.store.attempts_for("run1", "p5"), key=lambda row: row.attempt_no
        )
        self.assertIs(rows[0].retry_class, st.RetryClass.SEMANTIC)
        ...
        self.assertTrue(
            any("FALSIFICATION_NO_SUBJECT" in str(row.get("detail")) for row in audits),
            audits,
        )
