...
class CrossRunBudgetFixture(unittest.TestCase):
    ...
    def _run(self, argv):
        ...
        try:
            with (
                ...
                mock.patch.object(maestro.pv, "validate_plan", return_value=validation),
                mock.patch.object(maestro, "_plan_collector", return_value=None),
                mock.patch.object(
                    maestro,
                    "_validate_run_paths",
    ...
    def _reject(store, run_id, node_id, findings, *, blocked=False):
        """Record one content failure through the production lifecycle API."""
        detail = {
            "reason": "semantic verification rejected the candidate",
            "blocking_checks": [f"diff.check_{index}" for index in range(findings)],
        }
        if blocked:
            store.mark_blocked(
                run_id,
            ...
            store.fail_attempt(run_id, node_id, st.RetryClass.SEMANTIC, detail=detail)

    def _rejected_run(
        self, run_id, rejections, *, node_id=FIRST, digest=None, blocked=False
    ):
...
class TheCrossRunBudgetRefusesAFreshRunTest(CrossRunBudgetFixture):
    ...
    def test_the_nodes_are_sorted_by_node_id(self):
        ...
        self.assertEqual(
            [SECOND, FIRST], [entry["node_id"] for entry in payload["nodes"]]
        )
        self.assertEqual(
            sorted(entry["node_id"] for entry in payload["nodes"]),
...
class TheOperatorEscapeTest(CrossRunBudgetFixture):
    ...
    def test_its_use_is_recorded_as_a_typed_fact_in_the_ledger(self):
        ...
        self.assertEqual(
            {
                "cumulative_semantic_attempts": 6,
                "effective_ceiling": 3,
    ...
    def test_an_unnamed_node_is_still_refused(self):
        ...
        self.assertEqual(OUTCOME, payload["outcome"])
        self.assertEqual([SECOND], [entry["node_id"] for entry in payload["nodes"]])
    ...
    def test_the_escape_survives_the_configured_run_flag_rule(self):
        ...
        self.assertNotEqual("MAESTRO_CONFIGURATION_INVALID", payload["outcome"])
        self.assertEqual(HALTED, payload["outcome"], payload)
...
class TheCumulativeCountIsOnePredicateTest(unittest.TestCase):
    ...
    @staticmethod
    def _attempt(run_id, node_id, attempt_no, *, semantic=True):
        return st.AttemptRecord(
            ...
            turn_count=0,
            retry_class=(
                st.RetryClass.SEMANTIC if semantic else st.RetryClass.ENVIRONMENTAL
            ),
            extra={},
        )
    ...
    def test_it_sums_the_per_run_count(self):
        rows = {
            "run-a": (self._attempt("run-a", "n", 1), self._attempt("run-a", "n", 2)),
            "run-b": (self._attempt("run-b", "n", 1),),
        }
        self.assertEqual(
            (3, ("run-a", "run-b")), rp.semantic_attempts_across_runs(rows, "n")
        )

    def test_it_agrees_with_semantic_attempts_total_per_run(self):
        rows = {
            "run-a": (self._attempt("run-a", "n", 1), self._attempt("run-a", "n", 2)),
            "run-b": (self._attempt("run-b", "n", 1), self._attempt("run-b", "n", 2)),
        }
        total, _runs = rp.semantic_attempts_across_runs(rows, "n")
        self.assertEqual(
            sum(
                rp.semantic_attempts_total(attempts, "n") for attempts in rows.values()
            ),
            total,
        )
    ...
    def test_a_run_with_no_semantic_failure_is_not_named(self):
        ...
        rows = {
            "run-a": (self._attempt("run-a", "n", 1, semantic=False),),
            "run-b": (self._attempt("run-b", "n", 1),),
        }
        self.assertEqual((1, ("run-b",)), rp.semantic_attempts_across_runs(rows, "n"))

    def test_another_nodes_attempts_are_not_counted(self):
        rows = {"run-a": (self._attempt("run-a", "other", 1),)}
    ...
    def test_an_infra_failure_is_not_counted(self):
        ...
        rows = {
            "run-a": (
                self._attempt("run-a", "n", 1, semantic=False),
                self._attempt("run-a", "n", 2, semantic=False),
            )
        }
        self.assertEqual((0, ()), rp.semantic_attempts_across_runs(rows, "n"))
    ...
    def test_a_content_failure_that_is_not_a_rejection_is_counted(self):
        ...
        self.assertEqual((2, ("run-a",)), rp.semantic_attempts_across_runs(rows, "n"))
...
class TheReaderScopesByDigestAndExcludesTheCurrentRunTest(CrossRunBudgetFixture):
    ...
    def test_grants_are_summed_over_the_prior_runs(self):
        self._grant(self._rejected_run("run-aaa-first", 1, blocked=True), FIRST, 2)
        self._grant(self._rejected_run("run-bbb-second", 1, blocked=True), FIRST, 3)
