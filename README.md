...
Maestro places lane panes inside the already-open Herdr workspace for the target repository — the one the operator runs `uv run adws/maestro.py` from. The launcher finds that workspace by a live pane whose `cwd` resolves to the repository root; no match refuses (`WORKSPACE_UNRESOLVED:NO_REPOSITORY_WORKSPACE`), more than one refuses as ambiguous, and Maestro never creates a workspace of its own. Ambient `HERDR_WORKSPACE_ID` and focus are never placement authority. The run caption `<repository-basename>-<first-four-run-hash-characters>` (strip a leading `run-` first; preserve the basename's casing; FDAdb + `e892fe...` → `FDAdb-e892`) is display-only and used in completed session names; the full `run_id` is runtime identity.

Each active lane gets exactly one ordinary tab inside that workspace, labelled the exact authored `lane_id` (for example `lane-wp6-tests`), created only when the lane first dispatches an agent:

```text
herdr tab create --workspace <repository-workspace-id> \
  --label maestro-pending-<hash> --cwd <first-role-checkout> --no-focus
herdr tab rename <tab-id> <lane-id>     # after the seed pane is tagged
```

No linked child Space, no `herdr worktree open`.

Role panes are created only when that role first starts. Live pane labels are exactly `tester`, `tester-reviewer`, `builder`, `code-reviewer`, or `integration-reviewer` — do not repeat the run or lane. The first role replaces the tab's authenticated seed pane; later roles split only inside that same tab, each at its own checkout. Never split from `--current` or another lane's pane. All placement uses explicit workspace, tab, or pane IDs and `--no-focus`. Concurrent first launches resolve exactly one repository workspace and create exactly one tab per lane. Tokenless panes in a lane tab are the operator's and are never adopted, relabelled, or closed.
...
Scheduler restart and amendment rediscover from Herdr report-metadata (Maestro-owned source: full `run_id`, canonical repository fingerprint, `lane_id` where applicable, role where applicable, repository workspace ID, and lane tab ID) plus verified live IDs, pane cwd, agent ownership, and stable agent name. Labels are display text, not adoption proof; a user-renamed role pane whose metadata still names the role is relabelled. Only a confirmed `pane_not_found`, `workspace_not_found`, or `agent_not_found` recreates the missing expected object. Duplicate, label-only, wrong-workspace, wrong-tab, wrong-cwd, wrong-agent, dirty, unknown, or malformed candidates refuse; never adopt or rename them as current roles. Transport identity stays out of `lane_state` and artifact identity. Amendment closes the superseded revision's run-owned panes; lanes then create their tabs and panes lazily.

Keep role panes open while the run is executing, waiting, integration-review pending, publishable, or blocked by review/amendment. Cleanup may begin only after successful `MAIN_PUBLICATION` and derived COMPLETE. For every role: prove idle, send `/rename <repository-basename>-<short-run>-<lane-id>-<role>` as composer text plus Enter, wait for exact `Session renamed to "<full-session-name>".`, then close every authenticated run-owned role pane, then remove cwd. The repository workspace is the operator's and is never closed. Rename or close failure leaves that pane and cwd intact (for example `SESSION_RENAME_UNCONFIRMED`) without rolling back publication. Cleanup is idempotent.
