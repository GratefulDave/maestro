...
A launched handle is bound to its pane's terminal, not to the pane id it started in. `herdr pane move` across tabs or Spaces mints a new public pane id and preserves `terminal_id` and the agent session, so a binding re-verified after such a move re-resolves the handle's pane, tab, and workspace ids to the pane now carrying that terminal and continues; the lane child Space the launcher must close is not rewritten by a relocation. `BINDING_MISMATCH` remains the refusal when the terminal is gone, when the pane found under it is a different terminal, or when the named agent no longer sits in the pane the terminal resolved to. Rearranging a pane is a display act by the operator and is never a change of session.
