...
Concurrency is opt-in per deployment: `maestro.config.yaml` carries an integer `concurrency` of at least one, absent meaning one, and an install that does not set it does not change behaviour. At one, every stage runs on the calling thread. Above one, the ready non-merge lanes of a round are dispatched together and awaited as a set, and the first exception in submission order is raised once its siblings have finished their stage; the merge branch stays on the calling thread and advances one lane, so integration order is unchanged by concurrency. An interrupt pauses every in-flight lane, not one, and a lane that completed between the interrupt and the pause is refused by the ledger rather than double-recorded. An interrupt also *ends* every in-flight wait, within one poll interval, on whichever thread holds it: the signal reaches only the calling thread, so the handler snapshots what is executing, records the pause, and raises a process-wide stop that each wait on the dispatch path polls and restates as a typed operator-interrupt refusal. A worker that ignores it keeps the process alive after the run has already returned `WAITING`, because pool threads are joined at interpreter shutdown. The stop is a decision about the run and never a transport observation: a closed pane, an absent agent record and a quiet composer still end no wait. A wait already holding a declared envelope returns it rather than discarding it.
