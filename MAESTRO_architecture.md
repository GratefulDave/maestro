...
**Diagrams:** the whole mechanism on one page — every stage, the artifact or verdict that causes every transition, and the private-test boundary — is Figure 1 of [`docs/architecture/00-artifact-factory.html`](docs/architecture/00-artifact-factory.html), which also carries the durable-state topology; nine-stage machine — [`docs/architecture/00-lane-stage.html`](docs/architecture/00-lane-stage.html); persistent role topology — [`docs/architecture/00-role-topology.html`](docs/architecture/00-role-topology.html); where a lane's sealed suite is at each moment of its lifecycle, and where it is not — [`docs/architecture/00-sealed-suite-residence.html`](docs/architecture/00-sealed-suite-residence.html). The rest of [`docs/architecture/`](docs/architecture/index.html) is historical / superseded.
...
A materialized tree — the test-reviewer's private tree and every review, base and integration-gate tree the sealed suite runs in — is extracted with `git archive` and then made a git repository holding exactly one commit of exactly the extracted files (`hidden_vault._commit_materialized_tree`), so a repository test that asks git about its checkout, such as `git grep`, behaves as it does in a real one. The archive itself must still carry no `.git`; that refusal runs before the init. The repository has no remote, no history and no ref but `refs/heads/materialized`, and it is a directory, never a gitfile into the vault. Every git command in it runs with `GIT_DIR`, `GIT_WORK_TREE` and `GIT_CEILING_DIRECTORIES` bound to the tree, every inherited `GIT_*` variable dropped, and global and system config off. Before anything is cleared, extracted or initialised, the destination must lie strictly inside `runtime_state_root` and must not overlap the target checkout, its git dirs, the source repository, or the checkout the runtime copy runs from; otherwise `TreeContainmentRefused` and nothing is written.
...
- A build lane's own acceptance suite is absent from its checkout unconditionally, whatever its base carries. `LaneContext.sealed_private_paths` names the pair `_sealed_for` already resolves, and the launch removes those paths from the working tree on every turn: after an amendment the lane's new base is an integration head carrying the very suite it is graded against, and the merge cannot un-release it. The removal never reaches the candidate — it stays out of the index, and the same paths are subtracted from the commit pathspec. That subtraction is load-bearing, and it is no longer alone. A plan is refused `OUTPUT_OVERLAPS_TEST_SUITE` when any lane declares an output covering a `lane_kind=tests` lane's declared outputs, and `validate_declared_ownership` refuses `CANDIDATE_TEST_PATH_REFUSED` for a tree-delta entry naming one of the lane's own sealed paths, on the old path of a rename as well as the new. Before both, a shipped plan could declare such an output and `_commit_declared` would stage the removal as a deletion, `validate_declared_ownership` would admit that deletion as owned, and the next merge would delete a released suite from the integration ref. The subtraction still runs: these refuse a candidate that reached those paths anyway, rather than replacing it. Review measures the same invariant from the other side. Every site that materializes the accepted suite to run it hashes what is present against the accepted blob ids before the runner is invoked (`adw_modules/test_binding.py`), and there are three: the lane's code review, the base collection that decides whether a broken collection is the candidate's doing, and the run-level integration gate. All three, because a check on one of several sibling paths is how this factory has previously converted a loud refusal into a quiet failure blamed on the wrong actor. A mismatch refuses `TEST_SUITE_TAMPERED` as an environment fault and never as a candidate defect, and it escapes the base collection's absolution clause rather than being absorbed as "the fault predates the candidate". The `CODE_REVIEW` artifact and the integration gate's result each record the `test-suite.v1` digest of what ran, the former beside `sealed_digest`.
...
An authored build lane's merge also releases its predecessor tests lane's accepted suite. `_expected_merge_commit` overlays the sealed files onto the `merge-tree` result in a throwaway index, so one `INTEGRATION_MERGE` `after_sha` carries code and tests in the same tree and the final reviewer and `main` read both. The overlay is content-addressed: a crashed merge re-derives the same commit, and a re-derivation that forgets the suite refuses `INTEGRATION_REF_COLLISION`. Publication cannot be the seam — it is a CAS of `main` to an already-constructed SHA and builds no tree — so the release happens where a tree is built. Two cases are deliberately not released. An untyped lane's hidden meta-tests sit at paths of the tester's choosing and `REVIEWING_CODE` refuses an untyped candidate holding a file at any of them, so a released suite would refuse the lane's own rebuild after an amendment with `PRIVATE_PATH_COLLISION`. A tests lane no build lane consumes reaches `MERGED` at seal and emits no `INTEGRATION_MERGE`, so it has no merge to ride.

Zero-delta revalidation records `before_sha == candidate_sha == after_sha` with `revalidated=true` and creates no Git commit, and it applies only when the head already carries the released suite byte-for-byte. Otherwise the integration tree changes by exactly that suite and the merge writes a single-parent commit. If `BUILDER_OUTPUT.changed=false` and its base SHA is no longer the integration HEAD, append `BASE_INVALIDATION` and return to `BUILDING`.

Final reviewer evaluates the exact integration HEAD against the active plan revision, ordered public contracts, sealed lane tests, and architecture constraints named by the fingerprint. Its checkout carries every released suite and reads those files like any others, so a finding located at one of them is an observation and is recorded. Only an unconsumed tests lane's suite is absent, and the integration-reviewer role text says which is which. No verdict is rewritten before it is recorded.
...
| Accepted tests are sealed and hidden from implementation builders | Untyped `TESTS_SEALED → BUILDING`; authored `lane_kind=tests` `TESTS_SEALED → MERGED`; `SEALED_TEST_BUNDLE` contains vault digest/reference; private bytes stay absent from the builder worktree unconditionally, including the lane's own suite when its base already carries it, and absent from the run repo until the paired build lane's `INTEGRATION_MERGE` releases them; a typed build receives only the predecessor bundle ID/digest |
