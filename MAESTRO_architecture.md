...
A command the factory runs inside a provisioned tree runs in that tree's environment, never the operator's ambient one. Provisioning decides which files a tree contains; the environment a child is started with decides which interpreter, toolchain, and module set resolve them, and until both are the tree's the first is not a measurement of anything. Every child whose working directory is a factory tree and whose command resolves a language toolchain — the runner probe and version read, gate collection, gate execution, the provisioning command itself, the declared-interpreter release probe, and the participant in its candidate worktree — is started through one helper that drops the variables selecting an interpreter or toolchain (`VIRTUAL_ENV`, `PYTHONHOME`, `PYTHONPATH`, the conda pair, `NODE_PATH`, `NODE_OPTIONS`, `npm_config_prefix`, and their Rust and Go equivalents) together with the `bin` directory an activation prepended to `PATH`, and then prefers the tree's own `.venv/bin` and `node_modules/.bin`. Everything else the operator's shell carries — `PATH`, `HOME`, locale, SSH and Git identity, provider credentials, the §8.3 scratch redirects — passes through unchanged, because the factory depends on it. The failure this closes has the same shape as the bridged `node_modules` above and arrives by the opposite door: FDAdb run `d246ae95` failed its sealed suite `ModuleNotFoundError: bcrypt` in three consecutive review rounds against a tree whose own `.venv` had bcrypt, because the scheduler had been started under `uv run` and its `VIRTUAL_ENV` was copied into the runner's child. A harness environment fault was delivered to the builder as a verdict about its code. The agent pane is outside this boundary and is stated rather than worked around: `herdr agent start` has no environment option, the pane's shell is forked by the herdr server from a login shell, and the `--env` flags at pane split can set a variable but never unset one.
