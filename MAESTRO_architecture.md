...
Herdr topology is anchored to the already-open workspace for the target repository. The launcher selects it by finding a live pane whose `cwd` resolves to the repository root; zero matches refuses launch, and multiple matches refuse as ambiguous. Ambient workspace variables and focus are never placement authority, and Maestro never creates a replacement workspace. Each active lane owns one ordinary Herdr tab inside that repository workspace. Each role owns one pane inside its lane tab, with the pane cwd set to that role's checkout or private tree. Herdr workspace structure is UI organization only; role cwd, Git isolation, and publication authority remain exclusively those of the role tree.

Lane-tab creation is restart-safe. The launcher creates the tab with a deterministic provisional label, the explicit repository workspace ID, the first role checkout as cwd, and `--no-focus`; it authenticates the returned seed pane with repository/run/lane/parent metadata before renaming the tab to the authored `lane_id`. An interrupted provisional tab is recoverable only when its workspace, unique seed pane, compatible metadata, deterministic label, and checkout scope all match. An authored-label collision without ownership metadata refuses. UI placement never calls `herdr worktree open`.

Every later role launch splits a new pane inside the authenticated lane tab and assigns that pane its role checkout as cwd. The authenticated factory seed may be reaped after the first role pane exists; every other unlabeled or tokenless pane remains operator-owned. Complete Maestro report metadata is role identity. If that metadata names the expected role and every non-label live invariant also matches, a user-edited display label is repaired to the canonical role label before reuse. If metadata names another role, adoption refuses. Unowned panes are not role candidates and are never adopted, relabelled, or closed.

The selected repository workspace is the placement parent for the run. Existing owned role panes are reused only after exact run/repository/lane/role metadata, repository workspace, lane tab, role checkout scope, agent ownership, and stable agent name are proved. Unrelated panes are never moved or tagged. Concurrent controllers serialize first-tab creation per repository/run/lane so exactly one lane tab is created.

One lane tab is created lazily on first dispatch. Resume restores shells for already-materialized roles before scheduler construction and native gate collection, so a gate can block dispatch without hiding the run. Restoration does not start agents, send prompts, or replay completed work; surviving panes and sessions are reused and genuinely absent panes are recreated as owned shell containers. Role panes are labelled exactly `tester`, `tester-reviewer`, `builder`, `code-reviewer`, or `integration-reviewer`. Each receives a scratch-bound split with its own checkout cwd; tab placement never changes execution ownership. Never `--current`. Creation and splitting stay unfocused and Maestro never focuses an agent. Final `integration-reviewer` launches in the last topological integration-order lane when `_final_review` starts. `REVISE` resubmits to the existing proved session/cwd. Idle after output is not completion authority.
...
A role declares its turn finished by writing the requested envelope JSON to a `.part` sibling path and renaming it into place; a reader must never write into the declared path directly, because a poll landing mid-write reads a partial file as a failed turn. The launcher's `EXITED` poll state means the turn was declared, not that the process died — role sessions are persistent across turns, and process exit is never workflow authority. Where a route still writes in place, one bounded re-read of the envelope path after `EXITED` recovers a payload that settled after the poll. Cleanup begins only after successful `MAIN_PUBLICATION` and derived COMPLETE: idle proof and confirmed session rename for every available dispatched and restored live agent; only after every available rename confirms does it close every authenticated run-owned role pane, including panes whose agent record already expired, then remove role cwd. The rename is typed into an empty composer: the session's whole draft is cleared with the single interrupt it supports, whatever the cursor position and however many lines it holds, before `/rename` is sent, and input ingestion is waited for and any popup dismissed before Enter. A composer still holding a draft turns the slash command into the tail of an ordinary prompt, so no rename and no confirmation follow. Confirmation stays mandatory and an already-confirmed rename stays a no-op. The repository workspace, lane tabs as such, and unrelated tabs/panes are never closed. Rename or close failure leaves affected panes and cwd intact without rolling back publication. Cleanup is idempotent.

Scheduler restart first rediscovers candidates from Herdr report-metadata (Maestro-owned source: full `run_id`, canonical repository fingerprint, `lane_id` where applicable, role where applicable, repository workspace ID, and lane tab ID) and adopts only an exact metadata identity whose live IDs, pane cwd, agent ownership, and stable agent name all verify. Complete metadata is authoritative over mutable display text: an expected metadata role permits canonical role-label reconciliation before reuse, while a metadata-role mismatch refuses. It live-checks the process and resubmits the current stage from its immutable input. Only confirmed `pane_not_found`, `workspace_not_found`, `agent_not_found`, or process death permits recreation of the missing expected object. Empty labels, label-only matches, legacy stage/attempt panes, malformed observations, unreachable Herdr, mismatched placement, wrong-workspace, wrong-tab, dirty/out-of-scope trees, duplicates, and unproved agents are refused, never adopted or renamed as current roles. Transport identity stays out of `lane_state` and artifact identity. Never adopt a mutable branch ref, envelope, durable `actor_sessions`/generation row, or filesystem state as workflow authority.
...
| A linked child Space per lane (`herdr worktree open`), the invoking-workspace capture (`HERDR_WORKSPACE_ID`), UI anchor worktrees, and `agent focus` | `adw_modules/launcher.py`, `maestro.py` | a child Space placed lane panes outside the operator's repository workspace, where they could not be seen; a lane is one tab inside the repository workspace |
