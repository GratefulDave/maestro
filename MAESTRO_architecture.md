...
Concurrency is the default, not an opt-in: `maestro.config.yaml` carries an integer `concurrency` of at least one, and a deployment that declares none advances three independent ready lanes at once. Serial is the deviation and a deployment elects it by setting one, at which point every stage runs on the calling thread. Above one, the ready non-merge lanes of a round are dispatched together and awaited as a set, and the first exception in submission order is raised once its siblings have finished their stage; the merge branch stays on the calling thread and advances one lane, so integration order is unchanged by concurrency. An interrupt pauses every in-flight lane, not one, and a lane that completed between the interrupt and the pause is refused by the ledger rather than double-recorded.
